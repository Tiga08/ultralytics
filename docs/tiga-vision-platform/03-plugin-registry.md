...
插件注册名称须遵循以下规则：
- 全部**小写字母 + 下划线**，如 `regional_invasion`、`helmet_detection`
- 不允许空格、连字符或大写字母
- 名称在同类型中须全局唯一

## 内部实现说明

`PluginRegistry` 使用类变量 `dict` 存储注册表（`_detectors`、`_outputs`）。Python GIL 保证单次读写操作的基本安全性，适用于启动时注册、运行时只读的典型使用模式。若需支持运行时热加载（启动后动态注册新插件），应在注册和查询操作上加 `threading.Lock`。
...
- 生产环境只使用**一种注册方式**：要么显式导入，要么 `autodiscover`，不混用；`autodiscover` 仅用于第三方扩展包无法修改 `__init__.py` 的场景
